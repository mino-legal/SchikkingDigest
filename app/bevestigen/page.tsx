  ...
  if (!token) {
    return (
      <StatusPage title="Bevestigingslink ontbreekt" tone="error">
        <p>De link is niet compleet. Vraag een nieuwe bevestigingsmail aan via het inschrijfformulier.</p>
      </StatusPage>
    );
  }
  ...
  try {
    ...
    return (
      <StatusPage title="Je bent ingeschreven" tone="success">
        <p>
          Je inschrijving voor <strong>{subscriber.email}</strong> is bevestigd. Vanaf nu krijg je
          elke donderdag de wekelijkse SchikkingDigest in je inbox.
        </p>
        <p className="text-sm text-brand-darkgray/60">
          Onderaan elke e-mail staat een uitschrijflink — geen vragen, één klik.
        </p>
      </StatusPage>
    );
  } catch (err) {
